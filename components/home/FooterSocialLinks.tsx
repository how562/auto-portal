...
import { FOOTER_SOCIAL_LINKS, type FooterSocialPlatform } from "@/lib/footerSocial";
...
const ICONS: Record<FooterSocialPlatform, ReactNode> = {
  ...
  tiktok: (
    <svg viewBox="0 0 24 24" fill="currentColor" className="h-4 w-4" aria-hidden>
      <path d="M12.525.02c1.31-.02 2.61-.01 3.91-.02.08 1.53.63 3.09 1.75 4.17 1.12 1.11 2.7 1.62 4.24 1.79v4.03c-1.44-.05-2.89-.35-4.2-.97-.57-.26-1.1-.59-1.62-.93v7.15c0 4.55-3.69 8.24-8.24 8.24-4.55 0-8.24-3.69-8.24-8.24 0-4.55 3.69-8.24 8.24-8.24.45 0 .89.04 1.32.1v4.2c-.43-.15-.88-.23-1.34-.23-2.3 0-4.17 1.87-4.17 4.17s1.87 4.17 4.17 4.17 4.17-1.87 4.17-4.17V.02h3.91z" />
    </svg>
  ),
  linkedin: (
    <svg viewBox="0 0 24 24" fill="currentColor" className="h-4 w-4" aria-hidden>
...
interface FooterSocialLinksProps {
  /** Tighter row for footer bar (no top margin). */
  compact?: boolean;
}

export function FooterSocialLinks({ compact = false }: FooterSocialLinksProps) {
  return (
    <ul
      className={
        compact
